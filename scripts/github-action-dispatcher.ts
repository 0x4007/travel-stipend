...
async function enhancedScreenshot(page: Page, description: string, options: ScreenshotOptions = {}): Promise<void> {
  ...
  try {
    ...
    if (options.captureHtml) {
      ...
      appendFileSync(join(ARTIFACT_DIRS.html, `${baseFilename}.html`), html);
    }

    // Log DOM structure if requested
    if (options.logDOM) {
      const domStructure = await page.evaluate(() => document.documentElement.outerHTML);
      appendFileSync(join(ARTIFACT_DIRS.logs, `${baseFilename}-dom.txt`), domStructure);
    }
    ...
    if (options.dumpConsole) {
      ...
      appendFileSync(join(ARTIFACT_DIRS.logs, `${baseFilename}-console.log`), consoleLogs.join("\n"));
    }
...
async function handleError(page: Page, error: Error, context: string): Promise<void> {
  try {
    ...
    appendFileSync(join(ARTIFACT_DIRS.logs, "error-log.json"), JSON.stringify(errorDetails, null, 2) + "\n");
