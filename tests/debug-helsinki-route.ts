...
async function testHelsinki() {
  ...
  if (result.success && 'prices' in result) {
    ...
    const topFlightAvg = topFlightPrices.length > 0
      ? topFlightPrices.reduce((sum, price) => sum + price, 0) / topFlightPrices.length
      : 0;

    const allFlightAvg = allFlightPrices.length > 0
      ? allFlightPrices.reduce((sum, price) => sum + price, 0) / allFlightPrices.length
      : 0;
