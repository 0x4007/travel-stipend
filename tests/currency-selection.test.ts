...
describe('GoogleFlightsScraper Currency Selection', () => {
  ...
  it('should successfully change currency to USD', async () => {
    ...
    mockPage.evaluate.mockImplementation(async () => {
      callCount++;
      switch (callCount) {
        case 1: return null; // First call - check current currency (not USD)
        case 2: return true; // Second call - find and click currency button
        case 3: return undefined; // Third call - wait for dialog
        case 4: return 'Currency dialog content'; // Fourth call - get dialog content
        case 5: return true; // Fifth call - select USD
        case 6: return undefined; // Sixth call - wait for selection
        case 7: return true; // Seventh call - click save button
        case 8: return undefined; // Eighth call - wait for page update
        case 9: return true; // Ninth call - verify currency change
  ...
  it('should handle failure to find currency button', async () => {
    ...
    mockPage.evaluate.mockImplementation(async () => {
      ...
      switch (callCount) {
        case 1: return null; // First call - check current currency (not USD)
        case 2: return false; // Second call - fail to find currency button
        default: return undefined;
      }
    });
  ...
  it('should handle failure to select USD', async () => {
    ...
    mockPage.evaluate.mockImplementation(async () => {
      ...
      switch (callCount) {
        case 1: return null; // First call - check current currency (not USD)
        case 2: return true; // Second call - find currency button
        case 3: return undefined; // Third call - wait for dialog
        case 4: return 'Currency dialog content'; // Fourth call - get dialog content
        case 5: return false; // Fifth call - fail to select USD
        case 6: return false; // Sixth call - fail to select USD (second approach)
        default: return undefined;
      }
    });
